// example_vulnerable_http3.go demonstrates a race-prone voucher redemption server over HTTP/3.
//
...
//
// HTTP/3 SPECIFICS:
// HTTP/3 runs over QUIC, which uses UDP instead of TCP. This provides:
// - Multiplexed streams without head-of-line blocking
// - Built-in TLS 1.3 encryption
// - Connection migration support
// - 0-RTT connection establishment
//
// For race condition testing, HTTP/3's stream multiplexing can deliver
// multiple requests with even tighter timing than HTTP/2, making it an
// excellent protocol for demonstrating race vulnerabilities.
//
...
//
//	go run . -http3 -url "https://localhost:8443/redeem?code=DISCOUNT50" -count 10
package main
...
func main() {
	...
	// Generate self-signed certificate for TLS/QUIC
	cert, err := generateSelfSignedCert()
	if err != nil {
		log.Fatalf("Failed to generate certificate: %v", err)
	}

	// HTTP/3 server configuration using quic-go
	// Note: HTTP/3 requires TLS 1.3 and runs over QUIC (UDP)
	server := &http3.Server{
		Addr:    ":8443",
		Handler: mux,
	...
	log.Printf("Starting vulnerable HTTP/3 server on https://localhost:8443")
	log.Printf("Protocol: HTTP/3 over QUIC (UDP)")
	log.Printf("")
	...
	log.Printf("  go run . -http3 -url \"https://localhost:8443/redeem?code=DISCOUNT50\" -count 10")

	// ListenAndServe starts the QUIC/HTTP3 server
	// The TLSConfig is used to configure QUIC's built-in TLS 1.3
...
func handleRoot(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "Vulnerable Voucher Server (HTTP/3)\n")
