...
func main() {
	...
	var count int
	var delay int64
	var debug, prettify, useHTTP3 bool
	...
	flag.BoolVar(&useHTTP3, "http3", false, "Use HTTP/3 (QUIC) instead of HTTP/2 for the single packet attack")
	...
	// Protocol selection: HTTP/3 or HTTP/2
	if useHTTP3 {
		log.Printf("Using HTTP/3 (QUIC) mode")
		if logWriter != nil {
			fmt.Fprintf(logWriter, "  Protocol: HTTP/3 (QUIC)\n")
		}
		responses, err := runHTTP3Attack(req, count, delay)
		if err != nil {
			log.Printf("HTTP/3 attack failed: %v", err)
			if logWriter != nil {
				fmt.Fprintf(logWriter, "\nERROR: %v\n", err)
			}
			os.Exit(1)
		...
		analyzeResponses(responses, logWriter)
		return
	}

	// HTTP/2 mode (default)
	log.Printf("Using HTTP/2 mode")
	if logWriter != nil {
		fmt.Fprintf(logWriter, "  Protocol: HTTP/2\n")
	}
